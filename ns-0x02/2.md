...
是否默认设置：是
其他：一开始进行实验时是没有启用Siri的（因为平时不用），后来打开了Siri，因此认为实验是在默认设置下进行的
...
- 以上设计实现方式属于我们课堂上讲过的哪种强制访问控制模型？Lattice/BLP？Biba？
    - 属于Biba模型
    - 刚接收的短信属于还没有存入系统的，所有客人用户（低安全等级）都可以对其查看，属于**下读**行为；可以回复短信，属于**上写**行为
...
查阅资料得知可以通过访问控制机制防止特洛伊木马病毒，但具体的机理尚未理解
...
- OAuth在身份认证的基础上，还授予第三方应用访问受保护的信息的权限
...
例如，用户使用第三方应用程序时，选择使用Google登录
- 第三方应用首先通过OpenID证明Google上确实有这个用户
- 然后用OAuth token进一步获取访问用户在Google上的数据的权限，需要用户确认
- 第三方应用获取相应权限后，可以访问用户在Google上的部分数据
